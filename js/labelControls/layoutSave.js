...
com.logicpartners.labelControl.layoutSave = function(designer) {
	var self = this;
	...
	this.button = $("<button>Save</button>").css({ "line-height" : "30px" }).appendTo(this.buttonContainer)
		.on("click", function() {
			var zpl = self.designer.generateZPL();
			
			CurrentLayoutJson = zpl.zpl;
			LayoutSave(LayoutID,CurrentLayoutJson);
	
			var dialog = $("<div></div>").prop("title", "Save");
